 ...
 */
    ...
    void* alignedInputCopyPtr = inputDataCopy.get();
    CHECK(std::align(alignment, totalBytes, alignedInputCopyPtr, space));

    // Fill input with values
    auto* inputCopyPtr = reinterpret_cast<float*>(alignedInputCopyPtr);
    inputCopyPtr[0] = 1;
    inputCopyPtr[1] = 5;
    inputCopyPtr[2] = 2;
    inputCopyPtr[3] = 3;
    ...
    void* alignedOutputCopyPtr = outputDataCopy.get();
    CHECK(std::align(alignment, totalBytes, alignedOutputCopyPtr, space));
    auto* outputCopyPtr = reinterpret_cast<float*>(alignedOutputCopyPtr);
    std::fill_n(outputCopyPtr, numElements, -10.0f);

    InputTensors inputTensorsCopy
    {
        {0,armnn::ConstTensor(inputTensorInfo, alignedInputCopyPtr)},
    };
    OutputTensors outputTensorsCopy
    {
        {0,armnn::Tensor(runtime->GetOutputTensorInfo(netId, 0), alignedOutputCopyPtr)}
    };
    ...
    // Check the output is correct
    outputResult = reinterpret_cast<float*>(alignedOutputCopyPtr);
    CHECK(outputResult);
    CHECK(std::equal(outputResult, outputResult + numElements, expectedOutput.begin(), expectedOutput.end()));
